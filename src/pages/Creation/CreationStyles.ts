...
export const PreviewContent = styled.div<{ $background: string }>`
  width: 100%;
  height: 73.4%;
  background-color: white;
  border-radius: 40px 40px 48px 48px;
  background-image: ${(props) =>
    props.$background ? `url(${props.$background})` : `url(${photo})`};
  background-size: ${(props) => (props.$background ? '100% 55%' : '')};
  background-position: ${(props) => (props.$background ? '' : '50% 29%')};
  background-repeat: no-repeat;
`;

export const ContentDisplay = styled.div<{ $color: string }>`
  display: flex;
  flex-direction: column;
  height: 100%;
  align-items: center;
...
export const DisplayTopTime = styled.p<{ $dark: string }>`
...
`;
export const DisplayTopIndicators = styled.div<{ $dark: string }>`
  display: flex;
  gap: 5px;
...
export const DisplayBottomTitle = styled.p<{ $dark: string }>`
  margin-bottom: 8px;
  font-size: 20px;
  font-weight: 500;
  line-height: 32px;
...
export const DisplayBottomDescription = styled.p<{ $dark: string }>`
  display: inline-block;
  width: 100%;
  height: 184px;
  font-size: ${fontSizeNormal};
